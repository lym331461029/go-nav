	...
	virtual void handleClick(const float* s, const float* p, bool shift)
	{
		if (m_sample)
		{
			if (shift)
                ...
                float height = 2.0f;
                dtObstacleRef ref;
			    m_sample->addTempObstacle(p,radius,height,&ref);
			}
...
int Sample_TempObstacles::removeTempObstacleByRef(dtObstacleRef obRef)
{
    if (!m_tileCache)
        return -1;
    int code = m_tileCache->removeObstacle(obRef);
    if( code == DT_SUCCESS )
    {
        return 0;
    }
